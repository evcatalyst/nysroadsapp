...
- (NSString*)sgg_valueForPropertyAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    id cell_value = [info objectForKey:str];
    
    if ( [cell_value isKindOfClass:[NSArray class] ]) {
        cell_value = [NSString stringWithFormat:@"%@", cell_value];
        cell_value = [cell_value stringByReplacingOccurrencesOfString:@"\n" withString:@" "];
    }
